...
def load_hubmap_data_transductive(labeled_file, distance_thres, sample_rate, randseed = 1):
    df = pd.read_csv(labeled_file)
    #test_df = pd.read_csv(unlabeled_file)
    df = df.sample(n=round(sample_rate*len(df)), random_state=1)
    train_df, test_df = train_test_split(df, train_size = 0.7, random_state = randseed)
    #test_df = test_df.sample(n=round(sample_rate*len(test_df)), random_state=1)
    train_df = train_df.loc[np.logical_and(train_df['tissue'] == 'CL', train_df['donor'] == 'B004')]
    test_df = test_df.loc[np.logical_and(test_df['tissue'] == 'CL', test_df['donor'] == 'B004')] # Test is now from the same donor
    train_X = train_df.iloc[:, 1:49].values # node features, indexes depend on specific datasets
    test_X = test_df.iloc[:, 1:49].values
    
    labeled_pos = train_df.iloc[:, -6:-4].values # x,y coordinates, indexes depend on specific datasets
    unlabeled_pos = test_df.iloc[:, -6:-4].values # x,y coordinates, indexes depend on specific datasets
    #unlabeled_pos = test_df.iloc[:, -5:-3].values
    labeled_regions = train_df['unique_region']
    unlabeled_regions = test_df['unique_region']
    #unlabeled_regions = test_df['unique_region']
    train_y = train_df['cell_type_A'] # class information
    test_y = test_df['cell_type_A'] # class information
    cell_types = np.sort(list(set(df['cell_type_A'].values))).tolist()
    # we here map class in texts to categorical numbers and also save an inverse_dict to map the numbers back to texts
    cell_type_dict = {}
    inverse_dict = {}
    for i, cell_type in enumerate(cell_types):
...
def load_tonsilbe_data(filename, distance_thres, sample_rate, randseed = 1):
    df = pd.read_csv(filename)
    train_df = df.loc[df['sample_name'] == 'tonsil']
    train_df = train_df.sample(n=round(sample_rate*len(train_df)), random_state= randseed)
    ...
    cell_types = np.sort(list(set(train_y).union(set(test_y)))).tolist() # Change here
    cell_type_dict = {}
    inverse_dict = {}    
    for i, cell_type in enumerate(cell_types):
        cell_type_dict[cell_type] = i
